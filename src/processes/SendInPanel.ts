...
export default new Process('send in panel channel', async (client: Client, content: string) => {
    const panelChannel =
        client.channels.cache.get(process.env.panel) ??
        (await client.channels.fetch(process.env.panel).catch(() => {}));
    ...
    channel
        .send({
            embeds: [
                classic(client.user, { accentColor: true })
                    .setTitle('Erreur')
                    .setDescription(resize(`Quelque chose a été signalé :\n\`\`\`${content}\`\`\``))
            ]
        })
        .catch(log4js.trace);
});
