...
export default new ButtonHandler({
    ...
    if (!userPlaylists.size) return button.reply({
    ...
    if (userPlaylists.size > 1) {
        ...
        .setTitle("Playlist")
        .setComponents(
            row(new TextInputBuilder().setLabel('Nom de la playlist').setRequired(true).setStyle(TextInputStyle.Short).setCustomId('playlistName'))
        )
        ...
        const playlistTest = userPlaylists.find(y => y.name === findBestMatch(name, userPlaylists.map(x => x.name)).bestMatch.target)

        if (!playlistTest) return rep.reply({
    ...
    playlists.addToPlaylist({ userId: user.id, playlist, song: { id: res.tracks[0].id, url, title: res.tracks[0].title } })
