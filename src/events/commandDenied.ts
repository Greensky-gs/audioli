...
export default new AmethystEvent('commandDenied', (command, reason) => {
    if (command.interaction) {
        ...
        if (!!embeds[reason.metadata?.embedCode]) {
            ...
            log4js.trace(`Pas d'embed trouvé pour la dénietion de la commande ${command.command.options.name}\nPrécondition: ${reason.message} (message) ${reason.code} (code) ${JSON.stringify((reason.metadata ?? {}), null, 4)} (metadata)`)
            command.interaction.reply({
