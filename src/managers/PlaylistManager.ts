...
export class PlaylistManager {
    ...
    public playlistIncludes(
        playlist: { songs: { id: string; title: string }[] },
        song: string | { id: string; title: string }
    ) {
    ...
    public addToPlaylist({
        ...
        query(
            `UPDATE ${DatabaseTables.Playlists} SET songs='${JSON.stringify(list.songs).replace(
                /'/g,
                "\\'"
            )}' WHERE id='${list.id}'`
        );
    ...
    public shareWith(id: number, user: string) {
        ...
        query(
            `UPDATE ${DatabaseTables.Playlists} SET shared_with='${JSON.stringify(
    ...
    public async createList({
        ...
        const res = await query(
            `INSERT INTO ${
                DatabaseTables.Playlists
            } ( user_id, name, shared_with, songs, public, emoji ) VALUES ('${user}', "${sqlise(
                name
            )}", '[]', "${JSON.stringify(songs)}", '0', "${emoji ? GetEmojiStorage.process(emoji) : 'null'}")`
        );
    ...
    private async checkDb() {
        await query(
            `CREATE TABLE IF NOT EXISTS ${DatabaseTables.Playlists} ( user_id VARCHAR(255) NOT NULL, \`name\` VARCHAR(255) NOT NULL, songs LONGTEXT, shared_with LONGTEXT, public TINYINT(1) NOT NULL DEFAULT '0', emoji VARCHAR(255) DEFAULT NULL, id INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT )`
        );
        return true;
    }
