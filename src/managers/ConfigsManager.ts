...
export class ConfigsManager {
    ...
    private save(guild: string, key: configKey) {
        const confs = this.getConfigs(guild)
        const value = (x: keyof typeof confs) => typeof confs[x] === 'boolean' ? dbBool(confs[x]) : sqlise(confs[x].toString())
        query(`INSERT INTO ${DatabaseTables.Configs} ( ${Object.keys(confs).join(', ')} ) VALUES (
            ${Object.keys(confs).map((x: keyof typeof confs) => `"${value(x)}"`).join(', ')}
    ...
    private async fillCache() {
        ...
        res.forEach((value) => {
            ...
            Object.keys(value).filter(x => x !== 'guild_id').forEach((x: configKey) => {
    ...
    private async checkDb() {
        ...
        const generateTables = () => {
            return [{ name: 'guild_id', type: 'VARCHAR(255)', default: null, primary: true }, ...Object.keys(configsData).map((cf: keyof typeof configsData) => {
                return {
                    name: cf,
        ...
        const tableCreation = () => generateTables().map(x => `${x.name} ${x.type}${x.default ? ` DEFAULT "${sqlise(x.default)}"` : ''}${x.primary ? ' PRIMARY KEY' : ''}`).join(', ')

        await query(`CREATE TABLE IF NOT EXISTS ${DatabaseTables.Configs} ( ${tableCreation()} )`)
        const tables = await query<{Field:string, Type:string, Null:string, Key:string, Default: string | null, Extra: string}>(`SHOW COLUMNS FROM ${DatabaseTables.Configs}`)
        ...
        if (tables.length - 1 < Object.keys(configsData).length) {
            ...
            await Promise.all(missing.map(m => query(`ALTER TABLE ${DatabaseTables.Configs} ADD ${m} ${types[configsData[m].type]}${configsData[m].default ? ` DEFAULT "${sqlise(configsData[m].default)}"` : ''}`)))
        }
