...
export class ConfigsManager {
  ...
  public getConfigs(guild: string) {
    return (
      this._cache.get(guild) ?? { ...this.defaultConfigs, guild_id: guild }
    );
  }
  public getconfig(guild: string, config: configKey) {
    return this.getConfigs(guild)[config];
  }
  public setConfig<Key extends configKey>(
    guild: string,
    config: Key,
    value: (typeof configsData)[Key]["default"],
  ) {
  ...
  private save(guild: string, key: configKey) {
    ...
    const value = (x: keyof typeof confs) =>
      typeof confs[x] === "boolean"
        ? dbBool(confs[x])
        : sqlise(confs[x].toString());
    query(`INSERT INTO ${DatabaseTables.Configs} ( ${Object.keys(confs).join(
      ", ",
    )} ) VALUES (
            ${Object.keys(confs)
  ...
  private async fillCache() {
    const res = await query<configs<true>>(
      `SELECT * FROM ${DatabaseTables.Configs}`,
    );
      ...
      Object.keys(value)
        ...
        .forEach((x: configKey) => {
          if (getConfig(x).type === "boolean") {
            ...
            getConfig(x).type === "number" ||
            getConfig(x).type === "time"
          ) {
  ...
  private async checkDb() {
    ...
    const tableCreation = () =>
      generateTables()
        .map(
          (x) =>
            `${x.name} ${x.type}${
              x.default ? ` DEFAULT "${sqlise(x.default)}"` : ""
    ...
    if (tables.length - 1 < Object.keys(configsData).length) {
      const missing = (Object.keys(configsData) as configKey[]).filter(
        (x) => !tables.find((y) => y.Field === x),
      );

      await Promise.all(
        missing.map((m) =>
          query(
            `ALTER TABLE ${DatabaseTables.Configs} ADD ${m} ${
              types[configsData[m].type]
            }${
              configsData[m].default
                ? ` DEFAULT "${sqlise(configsData[m].default)}"`
                : ""
            }`,
          ),
        ),
