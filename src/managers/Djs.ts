...
export class DjsManager {
    ...
    public addDj(guild: Guild, user: string) {
        ...
        query(`INSERT INTO ${DatabaseTables.Djs} ( guild_id, list ) VALUES ('${guild.id}', "${this.mysqlise(list)}") ON DUPLICATE KEY UPDATE list="${this.mysqlise(list)}"`)
    }
    ...
    private async checkDb() {
        await query(`CREATE TABLE IF NOT EXISTS ${DatabaseTables.Djs} ( guild_id VARCHAR(255) NOT NULL PRIMARY KEY, list LONGTEXT )`)
    }
