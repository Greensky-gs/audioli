...
export default new AmethystCommand({
    ...
    if (cmd === 'déplacer') {
        ...
        if ([musicId, rapportId].some((x) => !playlist.songs.find((y) => y.id === x)))
            return interaction
                .reply({
    ...
    if (cmd === 'voir') {
        ...
        if (infos().length > 0) {
            embed.setThumbnail(
                infos().find((x) => x.url === playlist.songs[0].url)?.thumbnail ??
    ...
    if (cmd === 'retirer') {
        ...
        if (!playlist.songs.find((x) => x.id === songId))
            return interaction
                .reply({
                    embeds: [songNotInPlaylist(interaction.user)],
                    ephemeral: true
