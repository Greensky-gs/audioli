...
import { ApplicationCommandOptionType, ChannelType, ColorResolvable, ComponentType, Message, StringSelectMenuBuilder, TextChannel } from "discord.js";
import playlists from "../cache/playlists";
import { askEmoji, cancel, classic, created, deleted, error, playlistAlreadyExists, playlistVisibilityConflict, unexistingPlaylist, wait, visibility as visibilityEmbed, userBot, shareSelf, alreadyShared, sharedWith, notShared, unshared, noTracks, multipleTracks, songAdded } from "../contents/embeds";
...
export default new AmethystCommand({
    ...
    if (cmd === 'créer') {
        ...
        if (playlists.getUserLists(interaction.user.id).find(x => x.name === name)) return interaction.reply({
            embeds: [ playlistAlreadyExists(interaction.user, name) ],
            ephemeral: true
        }).catch(log4js.trace)
        const msg = await interaction.reply({
            embeds: [ classic(interaction.user, { question: true }).setTitle("Émoji").setDescription(`Souhaitez-vous mettre un émoji sur votre playlist ?`) ],
            components: [yesNoRow()],
            fetchReply: true
        ...
        if (emojiRes.customId === ButtonIds.Yes) {
            ...
            if (!selected || selected === 'cancel' || selected === "time's up") return interaction.editReply({
                embeds: [ cancel() ]
            }).catch(log4js.trace)
            emoji = selected;
        }
        ...
        const list = playlists.createList({
        ...
        if (!list) {
            log4js.trace(`Playlist not created.\nData: ${JSON.stringify({ name, user: interaction.user.id, songs: [] }, null, 4)}`)
    ...
    if (cmd === 'supprimer') {
        ...
        const confirmationMsg = await interaction.reply({
            embeds: [ classic(interaction.user, { question: true }).setTitle("Suppression").setDescription(`Êtes-vous sûr de supprimer la playlist ${playlist.emoji ? playlist.emoji : ''} ${playlist.name} ?`) ],
            components: [yesNoRow()],
            fetchReply: true
        }).catch(log4js.trace) as Message<true>
        ...
        if (!confirmation || confirmation.customId === ButtonIds.No) return interaction.editReply({
            embeds: [ cancel() ],
            components: []
        }).catch(log4js.trace)
    ...
    if (cmd === 'visibilité') {
        ...
        if (res === 'unexisting') return interaction.reply({
            embeds: [ unexistingPlaylist(interaction.user, playlist?.name) ],
            ephemeral: true
        }).catch(log4js.trace)
        if (res === 'already public' || res === 'not public') return interaction.reply({
    ...
    if (cmd === 'voir') {
        ...
        const embed = classic(interaction.user, { accentColor: true })
            .setTitle(`Playlist`)
            .setDescription(resize(`${playlist.emoji ? playlist.emoji + ' ': ''}${playlist.name}${playlist.user_id !== interaction.user.id ? ` (par ${pingUser(playlist.user_id)})` : ''}\n${numerize(playlist.songs.length)} musique${plurial(playlist.songs)}${playlist.songs.length > 0 ?
        ...
        await Promise.all(
            playlist.songs.slice(0, 29).map(promisifier)
        )
        ...
        if (infos().length > 0) {
            embed.addFields({
                ...
                value: msToSentence(infos().map(x => x.durationMS).reduce((a, b) => a + b, 0)) ?? 'N/A',
                inline: false
            })
        }
    ...
    if (cmd === 'partager') {
        ...
        if (user.bot) return interaction.reply({ embeds: [ userBot(interaction.user, user) ], ephemeral: true }).catch(log4js.trace)
        if (user.id === interaction.user.id) return interaction.reply({ embeds: [ shareSelf(interaction.user) ], ephemeral: true }).catch(log4js.trace)
        if (playlists.sharedWith(playlist.id, user.id)) return interaction.reply({ embeds: [ alreadyShared(interaction.user, user) ], ephemeral: true }).catch(log4js.trace)
    ...
    if (cmd === 'départager') {
        ...
        if (user.bot) return interaction.reply({ embeds: [ userBot(interaction.user, user) ], ephemeral: true }).catch(log4js.trace)
        if (user.id === interaction.user.id) return interaction.reply({ embeds: [ shareSelf(interaction.user) ], ephemeral: true }).catch(log4js.trace)
        if (!playlists.sharedWith(playlist.id, user.id)) return interaction.reply({ embeds: [ notShared(interaction.user, user) ], ephemeral: true }).catch(log4js.trace)
