...
export default new AmethystCommand({
    ...
    if (cmd === 'liste') {
        ...
        const pack =
            lists[rep.customId === ButtonIds.DJListMixed ? 'ur' : rep.customId === ButtonIds.DJListRoles ? 'r' : 'u'];
        ...
        if (pack.list.length <= pack.criteria) {
            ...
            pack.list.forEach((value, index) => {
                if (index % pack.criteria === 0 && index > 0)
                    embeds.push(pack.base(interaction.user, pack.list.length));
            ...
            new AmethystPaginator({
                ...
                invalidPageContent: (max) => ({
                    content: `Veuillez choisir un chiffre entre **1** et **${numerize(max)}**`,
                    ephemeral: true
                }),
                numeriseLocale: 'fr',
                modal: {
    ...
    if (cmd === 'ajouter') {
        ...
        const role = async () => {
            if (djs.getRoles(interaction.guild).find((x) => x.id === roleOption.id))
        ...
        const user = (roleTreated: boolean) => {
            ...
            if (djs.getUsers(interaction.guild).find((x) => x.id === userOption.id))
                return interaction[methodName]({
