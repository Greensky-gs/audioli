...
export default new AmethystCommand({
    ...
    if (cmd === 'déplacer') {
        ...
        if (selected === rapport) return interaction.reply({ephemeral: true, embeds: [moveSameSong(interaction.user)]}).catch(log4js.trace)
    ...
    if (cmd === 'recommencer') {
        ...
        interaction.reply({
            embeds: [
                play(interaction.user, current, interaction.guild.members?.me?.voice?.channel as VoiceChannel, false)
            ]
        }).catch(log4js.trace)
    }
    if (cmd === 'passer') {
