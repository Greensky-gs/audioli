...
export default new Precondition('is DJ')
.setChatInputRun(({ interaction }) => {
    ...
    const dj = () => {
        if (!djs.isDj(interaction.guild, interaction.user.id)) return {
            ok: false,
            type: 'chatInput',
            interaction,
            metadata: {
    ...
    const admin = () => {
        if (!(interaction.member as GuildMember).permissions.has('Administrator')) return {
            ok: false,
            type: 'chatInput',
            interaction,
            metadata: {
