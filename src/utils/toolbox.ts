import { ActionRowBuilder, AnyComponentBuilder, ButtonBuilder, ButtonStyle, Channel, ChannelType, Collection } from 'discord.js';
...
export const sqlise = (str: string | number | boolean) => typeof str === 'boolean' ? dbBool(str) : str.toString().replace(/"/g, '\\"');
...
export const numerize = (int: number | any[] | Collection<any, any>) => (typeof int === 'number' ? int : int instanceof Collection ? int.size : int.length).toLocaleString('fr')
