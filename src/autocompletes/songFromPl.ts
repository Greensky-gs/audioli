...
export default new AutocompleteListener({
    ...
    commandName: [
        { commandName: 'liste', optionName: 'musique' },
        { commandName: 'liste', optionName: 'rapport' }
    ],
        ...
        return queue
            .filter(
                (x) =>
                    ...
                    focusedValue.toLowerCase().includes(x.author.toLowerCase()) ||
                    x.author.toLowerCase().includes(focusedValue.toLowerCase())
            )
            .slice(0, 24)
            .map((x) => ({ name: resize(`${x.title} - ${x.author}`, 100), value: x.id }));
    }
});
